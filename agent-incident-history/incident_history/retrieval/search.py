...
class IncidentRetriever:
    def __init__(self, vector_store, top_k: int = 5, similarity_threshold: float = 0.65):
        self.vector_store = vector_store
        self.top_k = top_k
        self.similarity_threshold = similarity_threshold

    def search(self, query_vector: list[float], query_text: str = "") -> tuple[list[SimilarIncident], dict[str, object]]:
        ...
        # Filter by threshold or take highest scoring hit if available
        incidents = [
        ...
        # If threshold filtered everything out but raw hits exist, keep top 2 hits to guarantee evidence
        if not incidents and hits:
            incidents = [SimilarIncident.from_qdrant_hit(hits[0])]

        ranked = rank_incidents(incidents, query_text=query_text)
        logger.info(
